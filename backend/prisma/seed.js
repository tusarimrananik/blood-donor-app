...
const donors = donorBlueprints.map(([name, bloodGroup, area, latitude, longitude, donatedDaysAgo], index) => ({
    id: randomUUID(),
    name,
    bloodGroup,
    area,
...
const requests = requestBlueprints.map(([requesterName, requesterPhone, bloodGroup, message, area, hospital, urgency, status, donorIndex]) => ({
    requesterName,
    requesterPhone,
    bloodGroup,
    message,
